...
The vanila validation gave us accuracy of around 0.565 and error rate of around 0.435.
This is deffinetly the simplest approach we used which just splits data 70/30 allocating 70% for training and 30% of the data for testing.
The problem with this is that is the split is unlucky the accuracy can change a lot.
...
students <- data.frame(
  ...
  date = as.Date(c("11-12-2005", "14-12-2005", "09-07-2005"), format = "%d-%m-%Y")
)
...
# Unlock randomness for the actual data splitting
set.seed(NULL)
...
if(CV_choice == "Stratified"){


n <- nrow(heartData)
# Use the base R split for stratification:
split_prop <- 0.7
train_idx <- unlist(
  lapply(split(seq_len(n), heartData$hdc), function(idx) {
    sample(idx, size = round(length(idx) * split_prop))
  })
)
train_set <- heartData[train_idx, ]
test_set  <- heartData[-train_idx, ]
train_set$hdc <- factor(train_set$hdc, levels = levels(heartData$hdc))
test_set$hdc  <- factor(test_set$hdc, levels = levels(heartData$hdc))

  multinom_regression <- multinom(
    hdc ~ age + sex + place + chol + cp + trestbps +
      fbs + restecg + thalch + exang + oldpeak,
    data = train_set,
    trace = FALSE
  )
pred <- predict(multinomial_regression, test_set)
acc_strat <- mean(pred == test_set$hdc)
err_strat <- 1 - acc_strat
cat("Stratified validation Accuracy:", round(acc_strat, 3), "Error rate:", round(err_strat, 3), "\n")
}
...
After fitting the full logistic regression model with hdc01 as the binary outcome, we can see that the significant predictors considering p-value (p < 0.05) are:
- Sex
- Place: Switzerland
- place: VA Long Beach
- cpatypical angina
...
- oldpeak
```{r}
heartData$hdc01 <- ifelse(heartData$hdc == 0, 0, 1)
heartData$hdc01 <- factor(heartData$hdc01)

full_logit <- glm(hdc01 ~ age + sex + place + cp + trestbps + chol + fbs +
